...
pub fn router(state: AppState) -> Router {
    Router::new()
        ...
        .route("/v1/nodes/{id}/dispatch", post(dispatch_command))
        .route("/v1/results", get(list_results))
        .route("/v1/license", get(get_license).put(put_license))
        .route("/v1/skills", get(list_skills))
        // EE / report paths — closed without license (M7 stub).
        .route("/v1/ee/reports", get(ee_reports))
        .route("/v1/ee/reports/{id}", get(ee_report_by_id))
...
async fn dispatch_command(
    ...
    let run_id = body.run_id.unwrap_or_else(|| Uuid::new_v4().to_string());
    let params_json =
        serde_json::to_string(&body.params.unwrap_or(Value::Object(Default::default())))?;

    let msg = ServerMessage {
        request_id: Uuid::new_v4().to_string(),
        body: Some(server_message::Body::Dispatch(Dispatch {
            run_id: run_id.clone(),
            spec_id: body.spec_id.clone(),
            params_json: params_json.clone(),
        })),
    };

    let live = st.hub.send(&id, msg).await;
    if !live {
        st.db
            .enqueue_dispatch(&id, &run_id, &body.spec_id, &params_json)
            .await?;
    }
    ...
    Ok((
        ...
        Json(serde_json::json!({
            ...
            "delivered": if live { "live" } else { "queued" },
        })),
    ))
}
